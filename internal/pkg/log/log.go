...
// Logger 定义了 go-cicd-github-actions-docker 项目的日志接口. 该接口只包含了支持的日志记录方法.
...
// GetLogger 返回标准的全局Logger，用于传递给其他组件使用.
...
func (l *zapLogger) C(ctx context.Context) *zapLogger {
	...
	// if ip := ctx.Value(known.XIpWhoKey); ip != nil {
	// 	lc.z = lc.z.With(zap.Any(known.XIpWhoKey, ip))
	// }
